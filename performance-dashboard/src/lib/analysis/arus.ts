...
export function analyzeArus(
  ...
  const grandCallNode = findAllByLabel(roots, /^Jumlah kunjungan kapal$/i).find(
    (n) => n.unit === "Call",
  );
  const grandGTNode = findAllByLabel(roots, /^Jumlah kunjungan kapal$/i).find(
    (n) => n.unit === "GT",
  );
  ...
  const locations: LocationFigure[] = LOCATION_LABELS.map((rawLabel) => {
    const callNode = findAllByLabel(
      roots,
      exact(`Jumlah kunjungan kapal di ${rawLabel}`),
    ).find((n) => n.unit === "Call");
    const gtNode = findAllByLabel(
      roots,
      exact(`Jumlah kunjungan kapal di ${rawLabel}`),
    ).find((n) => n.unit === "GT");
    ...
    return {
      label: shortLocationLabel(rawLabel),
      rawLabel,
      call,
      sharePct: shareOf(call, cumCall) ?? 0,
      gt,
      gtPerCall: call > 0 ? gt / call : 0,
    };
  })
    .filter((l) => l.call > 0)
  ...
  const locationVoyage = locations.map((loc) => {
    ...
    const types = [
      ...VOYAGE_TYPES.map((vt) => ({
        label: vt.label,
        call:
          sumUnderMatchingHeaders(scoped, vt.match, "Call", throughMonth) ?? 0,
      })),
  ...
  const crossRows: CrossRow[] = locationVoyage.flatMap(({ loc, types }) =>
    types.map((t) => ({
      label: `${loc.label} · ${t.label}`,
      call: t.call,
      sharePct: shareOf(t.call, cumCall) ?? 0,
    })),
  );
  if (crossRows.length > 0 && cumCall !== null) {
    crossRows.push({
      label: "Total Regional 2 Banten",
      call: cumCall,
      sharePct: 100,
      isTotal: true,
    });
  ...
  const topLeadsBoth =
    ...
    VOYAGE_TYPES.every((vt) => {
      const ranked = locationVoyage
        .map(({ loc, types }) => ({
          label: loc.label,
          call: types.find((t) => t.label === vt.label)?.call ?? 0,
        }))
        .sort((a, b) => b.call - a.call);
      return (
        ranked[0]?.call > 0 && ranked[0].label === locationVoyage[0]?.loc.label
      );
    });
  ...
  const cargoRaw = [
    ...CARGO_LABELS.map((label) => ({
      ...
      value:
        sumByLabelAcrossTree(roots, exact(label), "Call", throughMonth) ?? 0,
    })),
    {
      label: "Tug Boat/Tongkang",
      value:
        sumByLabelAcrossTree(roots, TUG_BOAT_TONGKANG, "Call", throughMonth) ??
        0,
    },
  ].filter((c) => c.value > 0);
  ...
  const trendCall = Array.from(
    { length: throughMonth + 1 },
    (_, i) => grandCallNode?.months[i] ?? null,
  );
  ...
  const peakMoM =
    peak && peak.monthIndex > 0
      ? grandCallNode?.months[peak.monthIndex - 1]
      : null;
  const peakDelta =
    peak && peakMoM !== null && peakMoM !== undefined && peakMoM !== 0
      ? ((peak.value - peakMoM) / Math.abs(peakMoM)) * 100
      : null;

  // ---- Chart card conclusion subtitles ----
  const peakGTValue = peak
    ? (grandGTNode?.months[peak.monthIndex] ?? null)
    : null;
  const nextMonthIdx =
    peak && peak.monthIndex + 1 <= throughMonth ? peak.monthIndex + 1 : null;
  const nextMonthCall =
    nextMonthIdx !== null
      ? (grandCallNode?.months[nextMonthIdx] ?? null)
      : null;
  const nextMonthDeltaPct =
    nextMonthCall !== null && peak && peak.value !== 0
      ? ((nextMonthCall - peak.value) / peak.value) * 100
      : null;
  ...
  if (peak) {
    trendSubtitle = `Trafik memuncak di ${MONTH_NAMES[peak.monthIndex]} (${formatValue(peak.value)} kunjungan${
      peakGTValue !== null
        ? ` / ${formatValue(peakGTValue / 1_000_000, { decimals: 2 })} jt GT`
        : ""
    })`;
    trendSubtitle +=
  ...
  const topVoyage = voyageEntries.reduce(
  ...
  const voyageSubtitle =
    topVoyageShare !== null
      ? `${topVoyage.label} mendominasi ${formatValue(topVoyageShare, { decimals: 1 })}% trafik kunjungan.`
      : "Belum ada data komposisi jenis pelayaran.";
  ...
  const distribusiSubtitle =
    topLocation &&
    locations.length >= 2 &&
    topLocation.gtPerCall > locations[1].gtPerCall
  ...
  const biggerShipRatio =
    topLocation && locations[1] && locations[1].gtPerCall > 0
      ? topLocation.gtPerCall / locations[1].gtPerCall
      : null;
  const biggerShipNote: NoteBox | null =
    biggerShipRatio !== null &&
    biggerShipRatio >= 1.15 &&
    topLocation &&
    locations[1]
      ? {
          ...
          body: [
            ...
            {
              text: `${formatValue(biggerShipRatio, { decimals: 1 })}× lebih besar`,
              emphasis: "b",
            },
            {
  ...
  const crossTableSubtitle =
    topLeadsBoth && topLocation
      ? `${topLocation.label} memimpin di kedua jenis pelayaran dengan volume tertinggi.`
      : "Rincian kunjungan per lokasi & jenis pelayaran.";

  const lnGT = sumUnderMatchingHeaders(
    roots,
    /^Pelayaran Luar Negeri$/i,
    "GT",
    throughMonth,
    ...
    shareLNGT > shareLNCall * 1.3
      ? {
          ...
          body: [
            ...
            {
              text: `${formatValue(shareLNGT, { decimals: 1 })}% GT`,
              emphasis: "b",
            },
            {
  ...
  if (topLocation) {
    seg(" di lokasi ");
    seg(
      `${topLocation.label} (${formatValue(topLocation.sharePct, { decimals: 0 })}%)`,
      "b",
    );
  }
  if (peak) {
    ...
    seg(
      `${MONTH_NAMES[peak.monthIndex]} (${formatValue(peak.value)} kunjungan)`,
      "b",
    );
    seg(".");
  } else {
  ...
  if (isPartial) {
    seg(
      ` Data realisasi mencakup ${throughMonth + 1} dari 12 bulan (s.d. ${MONTH_NAMES[throughMonth]} ${year}).`,
      "i",
    );
  }
  ...
  const kpis: KpiCardData[] = [
    {
      ...
      unit: "Call",
      deltaText:
        throughMonth > 0 && cumCall !== null
          ? `Rata-rata ${formatValue(cumCall / (throughMonth + 1))} kunjungan / bulan`
          : undefined,
      deltaTone: "up",
      color: "green",
    ...
    },
    {
      label: "Gross Tonnage",
      value:
        cumGT !== null ? formatValue(cumGT / 1_000_000, { decimals: 2 }) : "—",
      unit: "juta GT",
      deltaText:
        gtPerCall !== null
          ? `${formatValue(gtPerCall, { decimals: 0 })} GT per kunjungan rata-rata`
          : undefined,
      deltaTone: "neutral",
      color: "blue",
      trackPercent: yearElapsedPct,
    },
    {
      ...
      deltaText:
        shareDN !== null
          ? `${formatValue(dn)} dari ${formatValue(voyageTotal)} kunjungan`
          : undefined,
      deltaTone: "up",
      color: "green",
    ...
    {
      label: peak
        ? `Puncak Trafik — ${MONTH_NAMES[peak.monthIndex]}`
        : "Puncak Trafik",
  ...
  if (shareDN !== null) {
    status.push({
      text: `Dalam Negeri mendominasi ${formatValue(shareDN, { decimals: 1 })}% (${formatValue(dn)} kunjungan) trafik total`,
      tone: "ok",
    });
  }
  if (topLocation) {
  ...
  if (cargoRaw.length >= 2) {
    ...
    if (top2Share !== null) {
      status.push({
        text: `${cargoRaw[0].label} & ${cargoRaw[1].label} menyumbang ${formatValue(top2Share, { decimals: 1 })}% kunjungan`,
        tone: "ok",
      });
    }
  }
  ...
  if (isPartial) {
    status.push({
      text: `Data realisasi baru mencakup ${throughMonth + 1} dari 12 bulan (s.d. ${MONTH_NAMES[throughMonth]} ${year})`,
      tone: "warn",
    });
  }
  ...
  return {
    ...
    badges: [
      ...
      cumGT !== null
        ? {
            value: `${formatValue(cumGT / 1_000_000, { decimals: 2 })} jt`,
            label: "Gross Tonnage (GT)",
          }
        : null,
      gtPerCall !== null
        ? {
            value: formatValue(gtPerCall, { decimals: 0 }),
            label: "GT per Kunjungan",
          }
        : null,
    ...
    trendSubtitle,
    voyageComposition: {
      labels: ["Dalam Negeri", "Luar Negeri", "Lainnya"],
      data: [dn, ln, lainnya],
    },
    voyageSubtitle,
    cargoRanking: {
      labels: cargoRaw.slice(0, 8).map((c) => c.label),
      data: cargoRaw.slice(0, 8).map((c) => c.value),
    },
    cargoSubtitle,
    locations,
    distribusiSubtitle,
    ...
    footerLeft:
      "LAP2026_Banten_ArusKapal — PT Pelabuhan Indonesia (Persero), Regional 2 Banten",
    footerRight:
      "Disiapkan: Juli 2026 · Kunjungan dalam Call, GT dalam satuan Gross Tonnage",
  };
}
