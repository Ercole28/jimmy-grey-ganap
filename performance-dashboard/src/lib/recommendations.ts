...
export const COLUMN_META: Record<RecommendationType, { label: string; accent: string }> = {
...
export function groupByColumn(cards: RecommendationCard[]): { type: RecommendationType; cards: RecommendationCard[] }[] {
  return (["op", "st", "rk"] as const).map((type) => ({ type, cards: cards.filter((c) => c.type === type) })).filter((g) => g.cards.length > 0);
}
